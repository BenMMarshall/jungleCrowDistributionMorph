...
<!--- MAKE SURE THAT JUNGLE CROW OR C MACRO IS USED PRIMARILY --->
...
Crows and ravens (Genus *Corvus*) exist across many different environments in their distribution across the globe, both as a group and generally within *Corvus* species (del Hoyo et al. 2016) <!-- MISSING -->.
...
The presence of geographical patterns of morphological variation in Jungle Crows has been noted for nearly a century [@hartert_types_1922], but these descriptions do not include the full species range.
...
Several studies, reinforced by field guides, already suggest that Jungle Crows differ significantly in morphology on regional scales [@dickinson_systematic_2004; @madge_crows_2010; @nakamura_postglacial_2016] and several species or subspecies divisions have been proposed [@klockenhoff_zur_1969; @martens_calls_2000].
...
Both reports refer to *C. macrorhynchos* as a species complex, but @klockenhoff_zur_1969 proposes 12 distinct subspecies with three areas of hybridization of which seven are mainland subspecies and five island subspecies (Figure 2-1) <!---- ADD REF TO FIGURE ---->.
...
@martens_calls_2000 suggests four full species, heavily relying on Klockenhoff's original boundaries as well as personal observations in the Himalayas [@martens_towards_1995] where vocal sampling was deficient (Figure 2-1). <!---- ADD REF TO FIGURE ---->
...
We predict that we will find morphological differences between populations on the Philippines and those on the mainland, as both proposals separate this population, and one genetic study supports this (Jønsson et al. 2012)<!-- MISSING -->.
...
print("<specimen locations figure here>")
```
...
All the photographs contained standardised reference scales (i.e., background grid or ruler) that enabled digital measures to be collected using imageJ<!-- MISSING -->.
...
Overall the @alamshah_distribution-wide_2025 dataset contained measurements from 1069 crows total (crows per museum: AMNH: 229, CUMV: 1, FMNH: 118, NHMUK: 550, USNM: 171).

## Analysis

In order to explore how patterns of morphological variation in the dataset corresponded to the proposed subspecies divisions of @klockenhoff_zur_1969, and @martens_calls_2000, we took their maps (Figures 2 and 4 in the respective papers) and used QGIS' georeferencing function (QGIS Development Team 2023)<!-- MISSING --> to convert the figure images to geospatial polygons.
...
We created priors for measures for Width at Nares (WN) (17mm), Height at Nares (HN) (22mm), total Exposed Culmen (ExCu) (62mm), Nares to Bill Tip (NtBT) (42mm) and Tarsus Length (TaLe) (57mm) by averaging the corresponding bill measurements provided by AVONET for *C. macrorhynchos* [@tobias_avonet_2022] and using those means as centres for normal priors with wide (20) standard deviations.
We set all priors to have a lower bound of 0.
For measures without prior data, we used the default uninformative priors generated by the *brms* package (Bürkner 2018, 2021<!-- MISSING -->) [@burkner_brms_2017].

We ran a total of ____<!-- CHECK NUMBER OF MODELS CREATED --> Bayesian Regression Models, one for each combination of group (subspecies model, sex) and measure (each morphological measure and the 4 composite measures).
...
*Corvus macrorhynchos culminatus*, *hainanus* and *levaillantii* x *macrorhynchos* showed differences in ExCu, whereas *levaillantii* x *tibetosinensis* only showed a difference in BBL.
*Corvus macrorhynchos hainanus* additionally showed a difference in HN.
...
```{r subspeciesPlots, echo=FALSE, message=FALSE, warning=FALSE, out.width="90%", fig.cap="The proposed subspecies separations from Klockenhoff and Martens. Points indicate the origin of measured museum specimens."}
...
```{r idwMaps, echo=FALSE, message=FALSE, warning=FALSE, out.width="90%", fig.cap="Maps showing an interpolated inverse distance weighted heatmap of all collected measures and the four combined measures. Thicker contours match the line breakdowns shown on the legends to the right of the maps, three thinner contours exist between each break. For Nares to bill tip:Exposed Culmen (NtBT : ExCu), a higher number means the nares fall closer to the head, a lower number means the nares fall closer to the tip of the bill. Measures were scaled and colored with yellow at the high end of the scale and dark green at the lower end."}
...
The northern India and Pakistan crows also exhibited another unique characteristic: their nares were located further forward on their bills (a low NtBT:ExCu ratio) compared to *C. macrorhynchos* from other areas.
...
Our exploration of morphological measurements in relation to climatic measures showed several significant relationships between our measurements and mean annual temperature, temperature seasonality, or precipitation, although most explained less than 10% of the variation in measurements (*R^2^* < 0.1) <!-- (Supplementary Figures 6-16) --> <!--- UPDATE R2 summary --->.
There were two notable exceptions.

In the ratio NtBT:ExCu (Fig. \@ref(fig:climateComparisonMapNtBTExCu)) we found a positive relationship between annual mean temperature and the NtBT:ExCu ratio `r NtBT.ExCu_temp_inText`, meaning as mean annual temperature increased, nares tended to fall more proximally on the bill, away from the tip.
...
```{r climateComparisonMapNtBTExCu, echo=FALSE, message=FALSE, warning=FALSE, out.width="90%", fig.cap="Measures of the ratio of Nares to Bill Tip : Exposed Culmen (NtBT : ExCu) in relation to annual mean temperature, temperature seasonality, and annual precipitation. Lower values are represented by dark green and higher values are represented by yellow."}
...
Factors we could not account for such as adult age, health, diet, and differing maturation rates between sexes may have influenced crow size <!-- CITES HERE -->, and the degree of sex dimorphism in our samples.
...
However, the low level of sexual dimorphism in our sample, with males being slightly larger on average, is not particularly unexpected as previous descriptions of this species have indicated males and females are indistinguishable in the field [@whistler_popular_2008].
...
This taken together with field guide descriptions and personal behavioural observations [Pers. Obs.; @alamshah2024morphological] suggests that *C. macrorhynchos* is a socially monogamous species, where both parents care for the young.
...
When plotting Height at Nares (HN) against climatic measures, we found a significant but weak negative relationship between HN and mean annual temperature (Supplementary Figure 12)<!-- SUPP FIGURE -->.
...
Tarsus Length (TaLe) followed a similar, albeit weaker, pattern to bill height, with a weak negative relationship between TaLe and annual mean temperature and a slightly stronger positive relationship between TaLe and seasonality (Supplementary Figure 14)<!-- SUPP FIGURE -->.
...
To uncover population level variation that would suggest species-level splits, genetic analysis across the species distribution would be required.
However, the results of our exploration highlight areas such as the Philippines, the Himalayas, and Northern Japan, that would benefit from targeted investigation via genetic sampling, vocal analysis, or additional morphometrics.
